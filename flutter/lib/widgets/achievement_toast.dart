...
// ---------------------------------------------------------------------------
// Unified queue-based toast system (achievements + quest completions)
// ---------------------------------------------------------------------------

/// Abstract entry so achievements and quests share one queue.
...
/// `showQuestCompletionToast` so achievements and quests never overlap.
void enqueueQuestToast(Quest quest) {
  _toastQueue.add(_QuestEntry(quest));
  _processQueue();
}
...
void _processQueue() {
  ...
  final Widget widget = switch (entry) {
    ...
    _QuestEntry(:final quest) => QuestToastWidget(
        quest: quest,
        onDismissed: dismiss,
      ),
  };
