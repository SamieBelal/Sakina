-- Freemium tier redesign — Lane C (Supabase migration)
--
-- Date: 2026-05-09
-- Spec: docs/superpowers/specs/2026-05-09-free-premium-tier-redesign-design.md
--
-- Implements the schema changes required by the value-first freemium model:
--
--   1. user_daily_usage gets a `discover_name_uses` column so the new
--      Discover Name 1/day cap can be tracked alongside reflect/built-dua.
--
--   2. user_profiles gets four new fields:
--        - warmup_reflect_remaining       INT DEFAULT 10
--        - warmup_built_dua_remaining     INT DEFAULT 10
--        - warmup_discover_name_remaining INT DEFAULT 5
--        - had_trial                      BOOLEAN DEFAULT FALSE
--      These power the lifetime warm-up budget (per-feature) and the
--      one-way `had_trial` latch that drops lapsed trialers straight
--      into the capped state regardless of remaining warm-up.
--
--   3. sync_all_user_data() is replaced (CREATE OR REPLACE) so the JSON
--      payload returned to the Flutter client on app launch surfaces
--      the new columns. Hydration on the client is what makes gating
--      work offline. No signature change — same return type, same args.
--
-- All ALTER TABLE statements use IF NOT EXISTS so the migration is
-- safely re-runnable. RLS for the affected tables is unchanged: the new
-- columns inherit the existing user-scoped policies on user_daily_usage
-- and user_profiles. No backfill needed — defaults cover existing rows.

-- 1. Add discover_name_uses to user_daily_usage
alter table public.user_daily_usage
  add column if not exists discover_name_uses int not null default 0;

-- 2. Add warmup counters + had_trial latch to user_profiles
alter table public.user_profiles
  add column if not exists warmup_reflect_remaining int not null default 10;

alter table public.user_profiles
  add column if not exists warmup_built_dua_remaining int not null default 10;

alter table public.user_profiles
  add column if not exists warmup_discover_name_remaining int not null default 5;

alter table public.user_profiles
  add column if not exists had_trial boolean not null default false;

-- 3. Update sync_all_user_data() to include the new fields.
--    Mirrors the wave5 definition exactly except for the daily_usage
--    and profile sub-objects, which gain the new columns.
