-- 2026-05-25: PR 5 of plan docs/superpowers/plans/2026-05-23-ai-bypass-token-spend.md
-- EXP-3 — IAP→sub upsell banner: hydrate two new client-side fields + add a
-- dismiss RPC.
--
-- The IapToSubUpsellBanner renders on the home surface when:
--
--   * user is NOT premium, AND
--   * lifetime_bypasses_purchased >= 6 (server-incremented by commit_ai_bypass
--     from PR 1; cancelled reservations do NOT count — only committed spends),
--     AND
--   * days_since_signup >= 7 (don't harass brand-new heavy IAP users), AND
--   * (iap_upsell_banner_dismissed_at IS NULL OR > 14 days ago) — CEO review's
--     14-day re-prompt cadence.
--
-- This migration does two things:
--
--   1. Re-issue sync_all_user_data() with the profile section extended by
--      `lifetime_bypasses_purchased` + `iap_upsell_banner_dismissed_at` so the
--      client can render the banner without round-tripping per render.
--   2. New SECURITY DEFINER RPC `dismiss_iap_upsell_banner()` so the close-tap
--      can write a server-side timestamp inside RLS (no direct UPDATE from the
--      authenticated client — keeps the surface immutable from client tampering
--      and atomically gives the 14-day suppression a single, monotonic clock).
--
-- The two columns themselves already exist (added in
-- 20260523000000_ai_bypass_reservations_and_rpcs.sql). lifetime_bypasses_purchased
-- is already incremented by commit_ai_bypass — PR 5 only wires it into the
-- sync payload + adds the dismiss handle.
--
-- Forward-only re-issue pattern, byte-for-byte identical to
-- 20260525000000_sync_all_user_data_add_first_bypass_fields.sql except the
-- profile jsonb_build_object now includes the two new fields. Mirrors the
-- established forward-only pattern.
...
-- ---------------------------------------------------------------------------
-- dismiss_iap_upsell_banner — close-tap handler for the IapToSubUpsellBanner.
--
-- Sets iap_upsell_banner_dismissed_at = now() for the authed user. Idempotent —
-- subsequent dismissals overwrite the timestamp, which resets the 14-day
-- suppression window. That's fine: the only way to dismiss again is to have
-- the banner re-appear after 14 days, then tap close again.
-- ---------------------------------------------------------------------------

create or replace function public.dismiss_iap_upsell_banner()
returns jsonb
language plpgsql
security definer
...
begin
  ...
  update public.user_profiles
    set iap_upsell_banner_dismissed_at = now()
    where id = current_user_id
    returning iap_upsell_banner_dismissed_at into v_dismissed_at;
