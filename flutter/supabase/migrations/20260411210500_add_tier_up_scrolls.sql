alter table public.user_tokens
  add column if not exists tier_up_scrolls integer not null default 0;

drop function if exists public.earn_scrolls(int);
drop function if exists public.spend_scrolls(int);
...
begin
  update public.user_tokens
  set tier_up_scrolls = tier_up_scrolls - amount
  where user_id = (select auth.uid()) and tier_up_scrolls >= amount
  returning tier_up_scrolls into new_balance;
