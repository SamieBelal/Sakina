-- Onboarding hard-paywall-after-tour gate: durable state for the entry latch
-- and the tour resume cursor, plus the server kill switch.
--
-- See docs/superpowers/plans/2026-06-03-hard-paywall-after-tour.md.

-- 1. Columns on user_profiles ------------------------------------------------
-- onboarding_paywall_cleared: the one-time entry latch. New rows default FALSE
-- (gated). Existing rows are BACKFILLED to TRUE so users who onboarded before
-- this feature are grandfathered and never flashed into the new gate.
alter table public.user_profiles
  add column if not exists onboarding_paywall_cleared boolean not null default false;

-- tour_step_index: resume cursor so a force-kill mid-tour reopens at the
-- abandoned step instead of restarting at 0.
alter table public.user_profiles
  add column if not exists tour_step_index integer not null default 0;

-- Grandfather every existing user (rows that predate this migration). The
-- ADD COLUMN ... DEFAULT false above set them all to false; flip them to true.
-- New rows created AFTER this migration keep the false default (gated).
update public.user_profiles set onboarding_paywall_cleared = true;

-- 2. Server kill switch ------------------------------------------------------
-- Seed disabled so the gate ships dark; flip to true to roll out, back to
-- false to instantly roll back (read with fallback=false in AppConfigService).
insert into public.app_config (key, value)
values ('hard_paywall_after_tour_enabled', 'false'::jsonb)
on conflict (key) do nothing;

-- 3. sync_all_user_data: surface the two new columns in the `profile` section
-- so a reinstall / second device boots the router gate from server truth.
-- IMPORTANT: the no-row FALLBACK returns NULL for both new keys (not a value),
-- so OnboardingGateService.hydrateFromProfile leaves the local cache untouched
-- when the row doesn't exist yet — it must never overwrite a brand-new user's
-- locally-set latch=false with a stray "cleared".
...
begin
  ...
  return jsonb_build_object(
    ...
    'checkin_history',
      coalesce(
        (select jsonb_agg(jsonb_build_object('checked_in_at', c.checked_in_at, 'q1', c.q1, 'q2', c.q2, 'q3', c.q3, 'q4', c.q4,
            'name_returned', c.name_returned, 'name_arabic', c.name_arabic) order by c.checked_in_at desc)
         from public.user_checkin_history c where c.user_id = current_user_id),
        '[]'::jsonb
      ),
    'reflections',
      coalesce(
        (select jsonb_agg(jsonb_build_object('id', r.id, 'saved_at', r.saved_at, 'user_text', r.user_text,
            'name', r.name, 'name_arabic', r.name_arabic, 'reframe_preview', r.reframe_preview, 'reframe', r.reframe,
            'story', r.story, 'verses', r.verses, 'dua_arabic', r.dua_arabic, 'dua_transliteration', r.dua_transliteration,
    ...
    'built_duas',
      coalesce(
        (select jsonb_agg(jsonb_build_object('id', d.id, 'saved_at', d.saved_at, 'need', d.need, 'arabic', d.arabic,
            'transliteration', d.transliteration, 'translation', d.translation) order by d.saved_at desc)
