-- Restore reflection `verses` in sync_all_user_data() (forward-only fix).
--
-- Date: 2026-05-10
--
-- Regression: 20260415125147_add_reflection_verses.sql added `'verses', r.verses`
-- to the reflections jsonb_build_object inside sync_all_user_data(). The freemium
-- redesign migration (20260509120000_freemium_tier_redesign.sql) reissued
-- CREATE OR REPLACE FUNCTION public.sync_all_user_data() to surface the new
-- warmup_*/had_trial profile fields, and silently dropped the `verses` field
-- from the reflections branch.
--
-- Effect on clients: on app launch the Flutter client calls sync_all_user_data
-- and rehydrates SharedPreferences from the JSON. Without `verses` in the
-- payload, the local reflections cache gets rewritten with empty verse lists —
-- a data-corruption regression for any user who reflects after launching the
-- new build.
--
-- Fix: forward-only migration. CREATE OR REPLACE the function with the EXACT
-- body shipped in the freemium-redesign migration, plus `'verses', r.verses,`
-- added back into the reflections jsonb_build_object between `'story'` and
-- `'dua_arabic'` (matching the original 2026-04-15 column order). All other
-- branches (xp, tokens, streak, daily_rewards, checkin_history, built_duas,
-- card_collection, profile w/ warmup fields, achievements, discovery_results,
-- daily_usage w/ discover_name_uses, daily_answers, quest_progress) are byte-
-- for-byte identical to 20260509120000.
