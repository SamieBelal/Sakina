...
-- HOTFIX: Previous migration (20260525000000) used non-existent
-- column `last_streak_date` on user_streaks (actual column is
-- `last_active`). Restoring to the correct column name byte-for-byte
-- with the PR2 baseline + PR4's two new profile fields.
...
begin
  ...
  return jsonb_build_object(
    'xp',
      coalesce(
        (select jsonb_build_object('total_xp', x.total_xp) from public.user_xp x where x.user_id = current_user_id),
    ...
    'daily_rewards',
      coalesce(
        (select jsonb_build_object('last_claim_date', dr.last_claim_date, 'next_day_to_claim', dr.next_day_to_claim, 'cycle_started_at', dr.cycle_started_at)
         from public.user_daily_rewards dr where dr.user_id = current_user_id),
        jsonb_build_object('last_claim_date', null, 'next_day_to_claim', 1, 'cycle_started_at', null)
      ),
    'reflections',
      coalesce(
        (select jsonb_agg(
          jsonb_build_object('created_at', r.created_at, 'name', r.name, 'name_arabic', r.name_arabic,
            'reframe', r.reframe, 'story', r.story, 'verses', r.verses,
            'dua_arabic', r.dua_arabic, 'dua_transliteration', r.dua_transliteration,
            'dua_translation', r.dua_translation, 'dua_source', r.dua_source,
            'related_names', r.related_names, 'user_text', r.user_text,
            'selected_emotions', r.selected_emotions, 'follow_up_answers', r.follow_up_answers,
            'off_topic', r.off_topic) order by r.created_at desc)
    ...
    'duas',
      coalesce(
        (select jsonb_agg(
          jsonb_build_object('created_at', d.created_at, 'category', d.category, 'arabic', d.arabic,
            'transliteration', d.transliteration, 'translation', d.translation, 'source', d.source)
          order by d.created_at desc)
         from public.user_duas d where d.user_id = current_user_id),
        '[]'::jsonb
      ),
    'card_collection',
      coalesce(
        (select jsonb_agg(
          jsonb_build_object('name_id', cc.name_id, 'tier', cc.tier,
            'discovered_at', cc.discovered_at, 'last_engaged_at', cc.last_engaged_at)
    ...
    'profile',
      coalesce(
        (select jsonb_build_object(
          'selected_title', p.selected_title, 'is_auto_title', p.is_auto_title,
          'created_at', p.created_at, 'warmup_reflect_remaining', p.warmup_reflect_remaining,
        ...
        jsonb_build_object('selected_title', null, 'is_auto_title', true, 'created_at', null,
          'warmup_reflect_remaining', 10, 'warmup_built_dua_remaining', 10,
          'warmup_discover_name_remaining', 5, 'had_trial', false,
          'first_bypass_consumed', false, 'display_name', 'Friend')
      ),
    'achievements',
      coalesce(
    ...
    'daily_usage',
      coalesce(
        (select jsonb_agg(
          jsonb_build_object('usage_date', u.usage_date,
            'reflect_uses', u.reflect_uses, 'built_dua_uses', u.built_dua_uses,
    ...
    'daily_answers',
      coalesce(
        (select jsonb_agg(
          jsonb_build_object('answered_at', da.answered_at, 'question_id', da.question_id,
            'selected_option', da.selected_option, 'name_returned', da.name_returned,
            'name_arabic', da.name_arabic, 'teaching', da.teaching,
            'dua_arabic', da.dua_arabic, 'dua_transliteration', da.dua_transliteration,
    ...
    'quest_progress',
      coalesce(
        (select jsonb_agg(
          jsonb_build_object('quest_id', q.quest_id, 'cadence', q.cadence, 'progress', q.progress,
            'completed', q.completed, 'period_start', q.period_start, 'updated_at', q.updated_at)
