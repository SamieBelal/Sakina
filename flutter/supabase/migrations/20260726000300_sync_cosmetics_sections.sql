-- Extend sync_all_user_data() to surface the cosmetics-economy sections.
--
-- ADDITIVE ONLY: the existing jsonb payload is reproduced byte-for-byte; the
-- only change is merging three new keys (noor, equipped, cosmetics) into the
-- returned object immediately before it is returned. No existing section is
-- dropped or altered. Signature / return / STABLE / SECURITY DEFINER /
-- search_path / grants are preserved (create or replace keeps grants).
