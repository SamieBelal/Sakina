...
The onboarding→tour→paywall funnel is ONE funnel segmented by feature-flag **super properties** (`flag_onboarding_trim`, `flag_hard_paywall`, `tour_variant`, `app_version`; `flag_tour_ab` retired 2026-07-25 — valid on historical events only, never re-add) — never separate event streams per flag. Before querying Mixpanel (project `4013350`) or adding funnel instrumentation, read [`docs/analytics/funnel-flags-and-querying.md`](./docs/analytics/funnel-flags-and-querying.md): it documents what each flag differentiates, the canonical event/property schema (incl. `placement` on paywall events, `step_id` vs `step_index` for cross-variant tour funnels), and the gotchas (test-ID exclusion, new events only populate post-release, identity is Simplified-ID-Merge). Add new event-name constants to `lib/services/analytics_event_names.dart`; emit from providers via the static `onAnalyticsEvent` hook pattern (no Riverpod in services).

**Reverse-trial A/B readout:** to read out the reverse-trial paywall experiment (control `control_no_trial` vs treatment `treatment_reverse_trial`, segmented by the `paywall_exp_arm` super property) and decide which arm to ship, follow [`docs/analytics/reverse-trial-experiment-readout.md`](./docs/analytics/reverse-trial-experiment-readout.md): it pins exactly **when to re-query** after the `1.2.0+4` release (T0 sanity → T0+21d decision read, min ~1,500 assigned/arm), **what metrics** to pull (primary = paid conversion per `experiment_assigned` user via RC `subscription_started`; guardrail = D7 retention), the **decision rule** (P(treatment>control) ≥ 95% AND no retention regression; tie → keep control), and how to **ship the winner** (control = flag flip; treatment = code change to make `assignPaywallArm` 100% treatment). Critical: the app-granted `trial_activated` is NOT a RevenueCat trial and never counts as revenue.
...
After the gacha reveal, the **deeper reflection** runs as **`BeatRevealFlow`** (`lib/widgets/beat_reveal/`) — a full-screen tap-through "beat" flow on the emerald **sacred canvas** (`AppColors.sacredCanvas*`), NOT the old card-based steps. `muhasabah_screen.dart` renders it for `DailyLoopStep.deeper`; the AI response is parsed into structured beats (`ReflectResponse.reframeKey/reframeBody/storyTitle/storyBeats/storySource/takeaway`) by the parser ladder in `ai_service.dart`, with `splitIntoBeats` as the legacy/fallback path. "Ameen" fires the quest hooks once and calls `completeDeeper()`. The same widget powers the Reflect result/off-topic screens (`reflect_screen.dart`, `includeVerses: true`). The old `_buildDeeper` step UI was deleted 2026-07-14 (see `docs/superpowers/specs/2026-07-14-bite-sized-ai-text-design.md`).
...
The live flow is the **trimmed ~20-page** variant (`onboarding_trim_enabled=true` in `app_config`); the legacy 27-page flow remains only as the flag-off rollback path. The rating gate screen is hardcoded in the page list (the old `Env.ratingGateEnabled` compile-time flag no longer exists). Canonical order is the source of truth in `onboarding_provider.dart` (`onboardingLastPageIndex` constant) and `docs/qa/ui-map.md` (stale — documents the legacy 27-page indices). Paywall flow lives at pages 22-26 (loader → plan → journey → rating gate → paywall) with the progress bar hidden — they have their own visual identity.
