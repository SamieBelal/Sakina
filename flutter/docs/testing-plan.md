...
**Refund-on-engage-throw (added 2026-04-26):** `collection_screen.dart:1174-1199` wraps `engageById` in `try/catch` after a successful `spendTierUpScrolls`. On throw, calls `earnTierUpScrolls(scrollCost)` to re-credit the balance and surfaces a `ScaffoldMessenger.showSnackBar` with "Couldn't upgrade. Your scrolls were refunded." The user can retry from a clean state — no scrolls lost. Real-world likelihood of an `engageById` throw is low (no network call; local cache + scoped prefs only), but the failure mode is now recoverable instead of silent.

**Quest-progress-on-tier-up coverage (added 2026-04-26):** `test/features/quests/tier_up_event_test.dart` (5 tests). Pins that `onCardTieredUp` appends exactly one ISO-8601 timestamp to the scoped `tier_ups_log_v1:<userId>` SharedPreferences key, that three calls produce three entries (no de-dup), that `tierUpsThisWeek` / `tierUpsThisMonth` correctly window-filter, and that the 200-entry cap drops oldest entries rather than refusing new writes (`quests_provider.dart:1166-1170`).
