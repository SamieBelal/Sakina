...
> **CORRECTED 2026-07-30 during Wave 4 groundwork — that precedence claim is only half true, and I repeated it here on the comment's authority without checking.** It holds for `/muhasabah`, a root-level route. It does **not** hold for `/duas`, which lives inside the `ShellRoute`: `_maybeShowDailyLaunch()` awaits a **network reconcile** and then pushes opaque on the **root** navigator in a post-frame callback, while the widget deep link is replayed after the first frame too — so whenever the reconcile resolves first, the overlay lands on top of the duʿā route. Today that costs one dismiss tap. **Under W4 it is materially worse: the overlay's primary action routes to `/muhasabah`, so a user who tapped a *duʿā* widget gets pulled in front of a question about their heart they never asked for** — exactly the auto-entry-as-hijacking failure this design exists to prevent. Wave 4 gates the push on the router's **current location at push time**, not merely on `mounted`, and corrects the stale comment. Pinned by an ordering test.
...
> **RESOLVED 2026-07-30 — there is now ONE notion, not three.** Wave 6 (`5527507`) made the widget send `mode: 'awaiting'` pre-reveal with **every Name field blank, including the anchor** (the anchor is that Name's teaching line and would have smuggled the claim back in through the copy); it renders "What's on your heart today?" instead, so widget, home CTA and question screen are one sentence. Wave 4 (`b368766`) deleted the overlay's Name card outright — both the streak≥1 rotation and the day-0 starter branch — rather than reconciling it. **The only surface that names a Name is now the reveal the user earned.**
>
> Two findings from that work are worth keeping. **A Dart-only fix would have been a no-op:** the Swift extension never read the payload's Name unless `mode == "personalized"` — otherwise it recomputed its own rotation from the bundled catalog — so the field this bullet told us to fix was dead data on iOS. And **showing the queue's real next Name cannot be made correct offline:** the widget provider pre-bakes a next-midnight entry and then sleeps, so it must produce tomorrow's Name unaided, without the server-side unseal or its 20-hour floor, and would still be wrong whenever the queue is exhausted and the reveal falls through to a gacha pull. There is no Name that can be shown pre-reveal and be true — which is why withholding, not predicting, was the answer.
