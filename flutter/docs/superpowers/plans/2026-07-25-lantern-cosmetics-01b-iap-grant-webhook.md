...
> **RESOLVED 2026-07-26** — I-1, I-2 and N-1 are closed by migration `20260726201000_cosmetic_iap_refund_edges.sql` + `supabase/tests/cosmetic_iap_refund_edges_test.sql` (39/39 green on a clean DB; clawback 12/12 and grant 13/13 unregressed), commit `b7f4f63`.
>
> **Deviation from the I-1 fix as originally specified, and why it is better:** the implementer DROPPED `created_inventory` as a precondition for the ownership delete rather than keeping it and merely adding the two skip conditions. Keeping it left sequence (b) half-fixed — refund A is skipped because B still stands, then refund B is *also* skipped because its `created_inventory = false`, so a user keeps a skin whose every purchase was refunded. Both stated skip conditions (another non-revoked grant for the same (user,item); `acquired_via <> 'iap'`) are still honoured. Rationale is in the migration header.
