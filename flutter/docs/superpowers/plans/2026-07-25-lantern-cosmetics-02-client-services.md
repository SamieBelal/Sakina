...
### OQ-1 (open question — spec is silent) — do trial/gift/referral users KEEP subscriber-perk skins?
§13 item 6 says "Decide whether trial/gift/referral users get the forever-kept monthly skin, and enforce it server-side," but the spec never states the ruling. **Lane B does NOT invent policy.** Task 7 implements the conservative, spec-safe behavior: premium-exclusive catalog rows are **equippable while any premium source is active** (readable/previewable always) but are **NEVER converted to permanent `user_cosmetics` ownership by the client** — permanent grants are a server decision (a future Lane A premium-grant-on-sync path). If the maintainer rules that trial/gift/referral users should permanently keep the monthly skin, that is a Lane A server change; Task 7 leaves a documented seam. **Controller: please rule on OQ-1 before the wardrobe (Lane D) ships equip UI for premium-exclusive rows.**
