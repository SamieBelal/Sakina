...
5. **The Day 1-7 promise gets an implementation owner (the orphaned Phase-2 seam).** The One Ship includes the minimum daily-loop seam: **D1-D7 daily reveal consults `user_name_queue`** (D1 reveal = Name #2's unseal deck — same surface, not a parallel one). A stated feeling overrides the Name, but the unseal deck is offered immediately after ("The Name we promised you is also waiting"). ~~The full feeling-first core-loop rewire (v1 Phase 2) is the first post-read item.~~ **[SUPERSEDED 2026-07-30 — D9 in the changes doc: the rewire rides the ship as W4, immediately after the seam wave. Spec: `../specs/2026-07-30-daily-loop-asks-design.md`.]** Without this seam, the plan screen and D1 push promise things Day 1 doesn't deliver — §G4 fails at its own test.
