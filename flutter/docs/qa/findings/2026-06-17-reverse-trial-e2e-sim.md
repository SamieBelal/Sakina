...
## Control-arm device test — VERIFIED (deterministic, via prefs surgery)
Two fresh signups both randomly bucketed treatment (buckets 85, 88), so the control arm was instead verified deterministically: a pre-created bucket-24 (`control_no_trial`) account was signed in, then placed at the post-tour gate via prefs (`onboarding_completed=true`, `onboarding_tour_v1_seen_<uid>=true`, `paywall_experiment_assigned:<uid>=true` so `appSession.paywallArm`→`control_no_trial`, app_config cache `post_tour_paywall_mode="soft"`, no trial window).
- On relaunch the router resolved `softPaywall` → the full-screen routing **`PaywallScreen` (placement `post_tour_soft`)** rendered with a close **X (dismissible = soft)** — the distinct control surface (treatment's Day-3 surface is the in-app `LapsedTrialSheet` instead).
- `paywall_viewed{placement:post_tour_soft}` fired; tapping the X routed to Home (free tier, no trial) and fired `soft_gate_dismissed{placement:post_tour_soft}`.
- Mixpanel (4013350) by `placement`: `paywall_viewed` post_tour_soft=1, `soft_gate_dismissed` post_tour_soft=1 (control) vs post_trial_soft=1 (treatment) — the two arms' surfaces segment cleanly.
- **Test-artifact caveat:** because the prefs-forced control user never ran `_completeOnboarding`, `recordPaywallArm` didn't set the `paywall_exp_arm` super-property, so these events don't segment by that super-property (the event-level `placement`/`arm` props do). A real control user sets the super-property normally at onboarding completion. The `experiment_assigned{control_no_trial}` + no-`activate_trial` logic remains covered by `reverse_trial_onboarding_test`.
